...
function storeOTP($conn, $Admin_id, $otp) {
    $otpHash = hash('sha256', $otp);
    $expiry = date("Y-m-d H:i:s", time() + 300); // OTP is valid for 5 minutes
    $sql = "UPDATE administrators SET OTP_hash = ?, OTP_expiry = ? WHERE Admin_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssi", $otpHash, $expiry, $Admin_id);
    $stmt->execute();
}

function sendOTP($email, $otp) {
    require __DIR__ . "/../Mailer.php";
    $mail->setFrom('admin@electionplateform.mydevhub.co.za', 'Election Plateform');
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $login = "SELECT Email, Admin_id, Password_hash FROM administrators WHERE Email = ? LIMIT 1";
    ...
    if ($admin && password_verify($password, $admin['Password_hash'])) {
        ...
        $otp = generateOTP();
        storeOTP($conn, $admin['Admin_id'], $otp);
        
        if (sendOTP($admin['Email'], $otp)) {
